...
<script>
    ...
    function updateEstimatedTime(distance) {
        $.ajax({
            url: 'http://127.0.0.1:5000/predict', // URL endpoint Flask
            type: 'POST',
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.error('Error fetching estimated time:', textStatus, errorThrown);
            }
        });
    }
    ...
    client.on('message', function(topic, message) {
        try {
            ...
            // Calculate the distance between the courier and the destination
            var distance = getDistance(lat, lng, destination[0], destination[1]);
            updateEstimatedTime(distance); // Call function to get estimated time
            ...
            // Update estimasi waktu kedatangan berdasarkan jarak
            // updateEstimatedTime(distance);
    ...
    // Call getUserLocation to set destination to user's location
    // getUserLocation();
</script>
